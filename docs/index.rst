...
The environment variable ``EPICS_CA_PUT_LOG_ADDR`` / ``EPICS_CA_JSON_PUT_LOG_ADDR``
...
``caPutJsonLogger`` is using Json as the output format. General format looks like ::
    ...
    <iocLogPrefix>{"date":"<date>","time":"<time>","host":"<client hostname>","user":"<client username>","pv":"<pv name>","new":<new value>,["new-size":<new array size>,]"old":<new value>[,"old-size":<old array size>][,"min":<minimum value>][,"max":<maximum value>][,"burst":<burst count>]}<LF>
...
Burst of scalar values: ::
    ...
    testIOC{"date":"2020-08-10","time":"13:08:44.144","host":"devWs","user":"devman","pv":"ao","new":8,"old":77.5,"min":7.5,"max":870.5,"burst"=10}<LF>
